...
func (repo *searchRepoImpl) SearchParagraphs(ctx context.Context, searchCriteria model.SearchCriteria) ([]model.SearchMatch, error) {
	...
	query := `SELECT w.volume, w.title, w.id, ts_headline('german', p.content, to_tsquery('german', $2), 'FragmentDelimiter=" ... ", MaxFragments=10, MaxWords=10, MinWords=3'), p.pages, p.id
		FROM paragraphs p 
