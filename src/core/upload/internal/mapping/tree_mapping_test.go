...
func testNestedHeadings(t *testing.T, sut *TreeMapperImpl) {
	...
	assert.Equal(t, model.H1, works[0].Heading.Level)
	...
	assert.Equal(t, 1, len(works[0].
		...
		Sections[0].
		Sections))
	assert.Equal(t, model.H9, works[0].
		Sections[0].
		Sections[0].
		Sections[0].
		Sections[0].
	...
	//     --8
	// --2
	assert.Equal(t, model.H1, works[1].Heading.Level)
	...
	assert.Equal(t, 2, len(works[1].
		Sections[0].
		Sections))
	assert.Equal(t, model.H5, works[1].
		Sections[0].
		Sections[0].
		Heading.Level)
	...
	assert.Equal(t, 1, len(works[1].
		...
		Sections[1].
		Sections))
	assert.Equal(t, model.H8, works[1].
		Sections[0].
		Sections[1].
		Sections[0].
		Heading.Level)
...
func testPureHuHeading(t *testing.T, sut *TreeMapperImpl) {
	...
	assert.Equal(t, "hu paragraph", works[0].Paragraphs[0])

	assert.Equal(t, 1, len(works[0].Sections))
	assert.Equal(t, model.H3, works[0].Sections[0].Heading.Level)
	assert.Equal(t, "h3 text", works[0].Sections[0].Heading.TextTitle)
}
...
func testTableExtraction(t *testing.T, sut *TreeMapperImpl) {
	...
	assert.Equal(t, "{table-extract}", works[0].Paragraphs[0])
	assert.Equal(t, "paragraph 1.1", works[0].Paragraphs[1])
	assert.Equal(t, "{table-extract}", works[0].Paragraphs[2])

	assert.Equal(t, 1, len(works[0].Sections))
	assert.Equal(t, 3, len(works[0].Sections[0].Paragraphs))
	assert.Equal(t, "paragraph 2.1", works[0].Sections[0].Paragraphs[0])
	assert.Equal(t, "{table-extract}", works[0].Sections[0].Paragraphs[1])
